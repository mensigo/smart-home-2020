...
public class UnknownEventHandler implements EventHandler {
    ...
    public void handleEvent(SensorEvent event, SmartHome smartHome) {
        if (event instanceof UnknownSensorEvent && event.getType().equals(UNKNOWN_EVENT)) {
            UnknownSensorEvent unknownEvent = (UnknownSensorEvent) event;
            SensorEventType type = unknownEvent.getType();
            String id = unknownEvent.getObjectId();
            String info = unknownEvent.getInfo();
            System.out.println("Event type [" + type + "] from object with id=" + id + " and info(" + info + ")");
        }
    }
}
